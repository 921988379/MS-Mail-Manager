...
def render_help_page():
    content = '\n<section class="section-stack">\n  <div class="toolbox-hero"><div class="toolbox-kicker">📘 Guide</div><h1 class="toolbox-title">使用指南</h1><p class="toolbox-desc">这里整理仓库和后台里已有的邮箱令牌、验证码、IMAP 与 API 工具。推荐流程：导入邮箱 → 检测令牌 → 综合检测 → 获取验证码/API 调用。</p></div>\n  <div class="quick-actions">\n    <div class="card"><h3>导入格式</h3><pre>邮箱----密码----应用ID----Refresh Token----辅助邮箱----辅助密码----分类</pre><p class="muted">辅助邮箱、辅助密码、分类可留空；也支持 |、逗号、Tab 分隔。保存后密码和 Refresh Token 会加密入库。</p></div>\n    <div class="card"><h3>快速判断账号是否挂了</h3><table><tr><th>现象</th><th>判断</th><th>建议</th></tr><tr><td>token_failed + 密码 IMAP 可读</td><td>令牌挂了，账号未必挂</td><td>继续用 IMAP 取码，重新 OAuth 授权刷新令牌</td></tr><tr><td>token_failed + 密码 IMAP 失败</td><td>账号/密码/风控可能异常</td><td>登录 Outlook 网页版验证或重新导入</td></tr><tr><td>Graph 可读</td><td>令牌和邮件权限可用</td><td>可正常用 latest-code</td></tr></table></div>\n  </div>\n  <div class="card"><h3>仓库里已有的工具能力</h3><table><tr><th>功能</th><th>入口</th><th>作用</th></tr><tr><td>刷新令牌工具</td><td><code>/tokens</code> / <code>/token_tool</code></td><td>Client ID + Refresh Token 换 Access Token，可选保存轮换后的 Refresh Token。</td></tr><tr><td>令牌状态检测</td><td>邮箱列表“检测状态”</td><td>检查 Refresh Token 是否还能换 Access Token，展示有效期、scope、错误。</td></tr><tr><td>最新验证码</td><td><code>/mails</code> / <code>/api/v1/latest-code</code></td><td>优先 Graph，失败后 XOAUTH2 IMAP，再用账号密码 IMAP 兜底。</td></tr><tr><td>账号综合检测</td><td>邮箱列表“综合检测”</td><td>统一检测 token、Graph、密码 IMAP 和最新验证码来源。</td></tr><tr><td>批量综合检测</td><td>邮箱管理 → 批量操作 → 综合检测</td><td>批量判断哪些账号 token 可用、Graph 可读、密码 IMAP 可读。</td></tr><tr><td>账号状态 API</td><td><code>/api/v1/account-status</code></td><td>给外部系统返回安全的综合状态摘要，不返回密码/token 明文。</td></tr></table></div>\n  <div class="card"><h3>状态说明</h3><table><tr><th>状态</th><th>显示</th><th>含义</th></tr><tr><td><code>token_ok</code></td><td class="ok">令牌可用</td><td>Refresh Token 能换 Access Token。</td></tr><tr><td><code>token_failed</code></td><td class="bad">令牌失效</td><td>Refresh Token 失效、scope 未授权或授权过期。</td></tr><tr><td><code>graph_ok</code></td><td class="ok">Graph 可读</td><td>Graph Mail.Read 能读取邮件。</td></tr><tr><td><code>xoauth2_imap_ok</code></td><td class="ok">OAuth IMAP 可读</td><td>Access Token 可用于 IMAP XOAUTH2。</td></tr><tr><td><code>imap_password_ok</code></td><td class="ok">密码 IMAP 可读</td><td>保存的邮箱密码可以通过 IMAP 读取邮件。</td></tr><tr><td><code>all_failed</code></td><td class="bad">全部失败</td><td>令牌、Graph、IMAP 通道均不可用。</td></tr></table></div>\n  <div class="card"><h3>常见错误</h3><table><tr><th>错误</th><th>含义</th><th>处理</th></tr><tr><td><code>AADSTS70000</code></td><td>请求的 scope 未授权或授权过期。</td><td>系统会先自动尝试“不指定 scope”兼容旧授权；仍失败时，重新登录并授权当前 Client ID/scope，同时用综合检测确认密码 IMAP 是否还能读。</td></tr><tr><td><code>invalid_grant</code></td><td>Refresh Token 失效、撤销、过期或账号安全状态变化。</td><td>重新 OAuth 授权获取新 Refresh Token。</td></tr><tr><td><code>User is authenticated but not connected</code></td><td>OAuth IMAP 认证通过，但邮箱未连接 Exchange mailbox。</td><td>尝试 Graph 或登录 Outlook 初始化邮箱。</td></tr><tr><td>密码 IMAP 失败</td><td>密码错误、IMAP 不允许、账号风控或邮箱不可用。</td><td>登录 Outlook 网页版验证、解除风控或重新导入正确密码。</td></tr></table></div>\n  <div class="card"><h3>外部 API 示例</h3><pre>curl -H "X-API-Key: ***" "https://token.seoyh.net/api/v1/latest-code?email=xxx@outlook.com"\ncurl -H "X-API-Key: ***" "https://token.seoyh.net/api/v1/latest-code?category=项目名"\ncurl -H "X-API-Key: ***" "https://token.seoyh.net/api/v1/account-status?email=xxx@outlook.com"\ncurl -H "X-API-Key: ***" "https://token.seoyh.net/api/v1/account-status?category=项目名"</pre><p class="muted">必须使用 <code>X-API-Key</code> 请求头；URL <code>?key=</code> 已禁用。<code>account-status</code> 需要 API Key 同时具备 <code>accounts</code> 和 <code>latest_code</code> 权限。</p></div>\n  <div class="card"><h3>安全提醒</h3><ul><li>接口和页面不会返回邮箱密码、Access Token、Refresh Token 明文。</li><li><code>saved_accounts.refresh_token</code>、<code>password</code>、<code>aux_password</code> 已加密存储。</li><li>不要丢失 <code>RTWEB_DATA_KEY</code>，否则加密字段无法解密。</li><li>账号密码 IMAP 只能兜底取邮件/验证码，不能稳定生成 Refresh Token。</li></ul></div>\n</section>'
