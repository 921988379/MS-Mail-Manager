...
class ActiveAccountPageTests(unittest.TestCase):
    def test_page_marks_active_email_and_thirty_second_poll(self):
        ...
        with patch.object(app, 'saved_accounts', return_value=rows):
            html = app.render_home_page('7')
        self.assertIn('当前邮箱：<b id="active-email">a@hotmail.com</b>', html)
        self.assertIn('/select?id=7', html)
        self.assertIn('setInterval(pollCodes, 30000)', html)
